from fastapi import FastAPI, Depends, HTTPException, Path, Form, status, Request
...
# Add admin IDs to the app state
app.state.ADMIN_IDS = [1, 4]
...
def admin_required(request: Request, current_user: User = Depends(current_user)):
    if current_user.id not in request.app.state.ADMIN_IDS:
...
async def create_order(order: OrderItem, db: Session = Depends(get_db), current_user: User = Depends(current_user)):
...
# Only admin can update the menu
